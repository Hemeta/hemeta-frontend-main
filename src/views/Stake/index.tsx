...
function Stake() {
  ...
  const hemeBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.heme;
  });
  const sHemeBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.sHeme;
  });
  const stakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.hemeStake;
  });
  const unstakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.sHemeUnstake;
  });
  ...
  const setMax = () => {
    if (view === 0) {
      setQuantity(hemeBalance);
    } else {
      setQuantity(sHemeBalance);
    }
  };
  ...
  const hasAllowance = useCallback(
    token => {
      if (token === 'HEME') return stakeAllowance > 0;
      if (token === 'sHEME') return unstakeAllowance > 0;
      return 0;
    },
    [stakeAllowance],
  );
  ...
  const trimmedSHemeBalance = trim(Number(sHemeBalance), 4);
  const stakingRebasePercentage = trim(stakingRebase * 100, 4);
  const nextRewardValue = trim(
    (Number(stakingRebasePercentage) / 100) * (Number(trimmedSHemeBalance) + Number(warmupBalance)),
    4,
  );
            ...
            <Grid item>
              <div className="card-header">
                <p className="single-stake-title">
                  HEME Staking ({String.fromCodePoint(0x1f9a6)}, {String.fromCodePoint(0x1f9a6)})
                </p>
                <RebaseTimer />
              </div>
            </Grid>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="stake-index">
                      ...
                      <Box component="p" color="text.secondary" className="single-stake-subtitle-value">
                        {currentIndex ? <>{trim(Number(currentIndex), 3)} sHEME2</> : <Skeleton width="150px" />}
                      </Box>
                    </div>
                  </Grid>
                </Grid>
            ...
            <div className="staking-area">
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <p className="desc-text">Connect your wallet to stake HEME2 tokens!</p>
                </div>
              ) : (
                <>
                  <Box className="stake-action-area">
                    ...
                    <Box className="stake-action-row" display="flex" alignItems="center">
                      ...
                      <TabPanel value={view} index={1} className="stake-tab-panel">
                        {address && hasAllowance('sHEME') ? (
                          ...
                          >
                            <p>{txnButtonText(pendingTransactions, 'unstaking', 'Unstake HEME2')}</p>
                          </Box>
                        ) : (
                          <Box
                            className="stake-tab-panel-btn"
                    ...
                    <div className="help-text">
                      {address && ((!hasAllowance('HEME') && view === 0) || (!hasAllowance('sHEME') && view === 1)) && (
                  ...
                  <div className={`stake-user-data`}>
                    {Number(warmupBalance) > 0 && (
                      <div className="data-row">
                        ...
                        <p className="data-row-value">
                          {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(warmupBalance), 4)} HEME2</>}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? (
                          ...
                          <>{new Intl.NumberFormat('en-US').format(Number(trimmedSHemeBalance))} sHEME2</>
                        )}
                      </p>
                    </div>
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? <Skeleton width="80px" /> : <>{nextRewardValue} sHEME2</>}
                      </p>
                    </div>
