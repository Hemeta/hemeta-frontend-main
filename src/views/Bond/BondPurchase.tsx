...
function BondPurchase({ bondKey, slippage }: IBondPurchaseProps) {
  ...
  const bondUnit = bond.autostake ? 'sHEME' : 'HEME';
  ...
  return (
    <Box display="flex" flexDirection="column">
      <div className={`${styles.input} input-container`}>
        ...
        {hasAllowance() ? (
          <Box
            className="transaction-button app-hemeta-button"
        ...
        bond.autostake && (
          <div className="help-text">
            <p className="help-text-desc">
              Note: The (4, 4) bond will stake all HEMEs at the start, so you will earn all rebase rewards during the
              vesting term. Once fully vested, you will only be able to claim sHeme.
            </p>
          </div>
        )
      ) : (
